...
- **适配器分发 = CLI 内嵌**（`ousheng adapter install [--dir <代码仓>] [--workspace <工作区>] [--actor <AI 窗口身份>]`）：幂等写出**五件套**——plugin/package.json/opencode.json + `.opencode/ousheng.json`（机器本地 `{workspace, actor}`，自动 gitignore，plugin 单一事实源）+ AGENTS.md 机器中立协议段（`<!-- ousheng:begin/end -->` 标记段自动维护，段外不触碰）；**座位资产（除 AGENTS.md 协议段）全走 `.opencode/.gitignore`**——二进制内嵌分发为单一来源，入库副本必漂移，且同事 clone 后开 opencode 会被插件报错（档案 §13）；外加**全局放行**（`~/.config/opencode/opencode.json` 合并 `permission.external_directory[<workspace>/**]=allow`——external_directory 默认 ask 且 TUI always 只活会话，不配则每会话弹权限询问；只增不删，解析失败/非对象结构不碰用户配置，档案 §11）；**升级二进制后每个代码仓重跑一次即同步**（workspace 记忆在 ousheng.json，无需再传）。PM 机 AI 会话身份 ≠ me 时用 `--actor`（human 型 me 只留给终端亲操）。锁：`adapters/opencode/adapter_test.go` + `cmd/ousheng/adapter_cmd_test.go`。
...
- **延后清单（记判据防丢）**：system list 加 open work 计数列（判据：真有人被僵尸系统坑过）；system 生命周期 schema（判据：派生可见性证明不够）；**agent↔系统 n:1 严格化**（判据：多 agent 多系统出现归属/告警歧义）；**人↔角色 1:1 结构化**（判据：多人协作真实发生，或拍板门需按角色校验；附带 roles.yaml 半实现待补）；**多项目人身份约定**（判据：第二个项目上绳）——详见档案 §20。
