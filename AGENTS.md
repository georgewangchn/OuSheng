...
- **S7 多机回归**：`scripts/dogfood-s7.sh` 四机拓扑（PM/后端/前端/测试 + bare 中央仓）全流程演练——含 C2 攻防（agent 自 ack 被拒→human ack 放行）、push 分歧 pull --rebase 恢复、四机 converge 一致性；`scripts/dogfood-s7-chaos.sh` 混沌版（同拓扑 + 第五机 mobile-agent + 12 突发事件：需求变更/事故插队/同单跨机冲突/机器损毁 re-clone/新成员移交/误取消重开/competing contracts/依赖被砍/伪造证据/紧急回滚/虚报被抓/优先级重排）。改 state/sync/activity 相关代码后必跑（两剧本都跑）。
