...
- **适配器分发 = CLI 内嵌**（`ousheng adapter install [--dir <代码仓>] [--workspace <工作区>] [--actor <AI 窗口身份>]`）：幂等写出**五件套**——plugin/package.json/opencode.json + `.opencode/ousheng.json`（机器本地 `{workspace, actor}`，自动 gitignore，plugin 单一事实源）+ AGENTS.md 机器中立协议段（`<!-- ousheng:begin/end -->` 标记段自动维护，段外不触碰）；外加**全局放行**（`~/.config/opencode/opencode.json` 合并 `permission.external_directory[<workspace>/**]=allow`——external_directory 默认 ask 且 TUI always 只活会话，不配则每会话弹权限询问；只增不删，解析失败/非对象结构不碰用户配置，档案 §11）；**升级二进制后每个代码仓重跑一次即同步**（workspace 记忆在 ousheng.json，无需再传）。PM 机 AI 会话身份 ≠ me 时用 `--actor`（human 型 me 只留给终端亲操）。锁：`adapters/opencode/adapter_test.go` + `cmd/ousheng/adapter_cmd_test.go`。
