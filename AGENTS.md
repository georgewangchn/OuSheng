...
- **S7 多机回归**：`scripts/dogfood-s7.sh` 四机拓扑（PM/后端/前端/测试 + bare 中央仓）全流程演练——含 C2 攻防（agent 自 ack 被拒→human ack 放行）、push 分歧 pull --rebase 恢复、四机 converge 一致性；改 state/sync/activity 相关代码后必跑。
